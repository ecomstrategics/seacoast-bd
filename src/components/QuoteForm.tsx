  ...
  return (
    <form action={formspreeUrl} method="POST" onSubmit={handleSubmit} className="grid gap-4 rounded-3xl bg-white p-6 shadow-soft">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="grid gap-2 text-sm font-semibold text-navy">
          Service Needed
          <select name="service" className="rounded-xl border border-navy/15 px-4 py-3">
      ...
      {status === "success" && <p className="rounded-xl bg-teal/10 px-4 py-3 text-sm font-semibold text-teal">Thanks — your quote request was sent. We’ll follow up shortly.</p>}
